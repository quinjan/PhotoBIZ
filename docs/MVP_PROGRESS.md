...
- `dotnet test services/api/tests/PhotoBIZ.Api.Tests/PhotoBIZ.Api.Tests.csproj --configuration Release --verbosity minimal` passed on 2026-05-17: 39 API/workflow tests. Clean builds may still emit the existing CA1848/CA1873 warnings in `PhotoBizInfrastructure.cs` for bootstrap logging.
...
- `dotnet test PhotoBIZ.slnx --configuration Release --verbosity minimal` passed on 2026-05-17: API 39 tests, Worker 1 test, Windows Agent 7 tests.
- `npm run build` from `apps` passed on 2026-05-17 for both `admin-web` and `booth-ui`.
- `npm run test:ci` from `apps` passed on 2026-05-17: Admin Web 7 tests and Booth UI 5 tests.
...
- Admin Web has mockup-aligned login and Application Owner dashboard/subscription-catalog/client/client-detail/subscription-audit views, plus dedicated setup, user, booth/offer/payment, cashier POS, reports, and audit views for client/cashier MVP workflows.
